...
//    }
    ...
    public function localUpload($field, $request, $tag = 'files', $size = 10 * 1024 * 1024, array $mimeType = ['image/jpeg', 'image/png', 'image/gif']) {
...
//            $result[$idx] = ErrorCode::ATTACHMENT_SIZE_EXCEEDED;
        ...
        try {
            ...
            $ratio = isset($request['compress_ratio']) ? (int)$request['compress_ratio'] : 0;
            if($ratio != 0) {
                $info = null;
                if(($info = getimagesize($real_path)) != null) {
                    Image::make($real_path)->resize($info[0] / 3, $info[1] / 3)->save($real_path);
                }  
            }
