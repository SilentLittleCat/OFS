...
EOF;
    ...
    public function single2($folder, $id, $name="data", $file="", $option=[]){
        ...
        $option['callback'] = isset($option['callback']) ? $option['callback'] : "";
        $option['watermark'] = isset($option['watermark']) ? $option['watermark'] : "";
        $compress_ratio = isset($option['compress_ratio']) ? $option['compress_ratio'] : 0;
        $hide = isset($option['hide_text']) ? $option['hide_text'] : false;
        if(!$option['callback'] && !empty($img)) {
            ...
            $imgHtml .="<li >
                    ...
                    <input type=\"hidden\" name=\"{$name}\" value=\"{$img['alt']}\">
                    </li>";
        }
        ...
        $html = <<<EOF
           <script type="text/javascript" src="/base/plugins/imguploader/uploader.js"></script>
            ...
            new upload({
                ...
                UploaderServer:"/admin/Base/Attachment/webupload?elementid=&watermark={$option['watermark']}&folder=$folder&_time=" + Math.random() + "&compress_ratio=$compress_ratio",
                UploaderSingle:true
            });
        </script>
EOF;
...
EOF;
    ...
    public function single4($folder, $id, $option = [])
    {
        $folder = urlencode($folder);
        $uploader = $id . '_uploader';
        $url = isset($option['url']) ? $option['url'] : 'empty';
        $upload_url = route('webupload') . "?folder=" . $folder;
        foreach($option as $key => $val) {
            $upload_url = $upload_url . '&' . $key . '=' . $val;
        }
        $token = csrf_token();
...
<div id="uploader-container">
    <div id="sg-upload-show" align="center"></div>
    <div id="sg-upload-info" align="center"></div>
    <div class="weui-btn weui-btn_primary" id="sg-upload-btn">选择图片</div>
    <input type="file" name="file" accept="image/*" id="upload-file-input" class="sg-hide">
    <input type="hidden" name="poster" id="upload-file-url" class="sg-hide">
</div>
<style type="text/css">
    #upload-file-input {
        display: none;
...
<script type="text/javascript">
    $('#sg-upload-btn').on('click', function() {
    ...
    $('#upload-file-input').change(function() {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#sg-upload-show').empty();
                html = '<img src="' + data.fileurl + '" width="200">'
                $('#sg-upload-show').append(html);
                $('#upload-file-url').val(data.fileurl);
                $('#sg-upload-info').text('上传完毕...');
            },
            error: function (data) {
                console.log('Error:', data);
                $('#sg-upload-info').text('上传失败...');
            }
        });
    });
</script>
...
EOF;
    ...
    public function single5($folder, $id, $option = [])
    {
        $folder = urlencode($folder);
        $uploader = $id . '_uploader';
        $url = isset($option['url']) ? $option['url'] : 'empty';
...
<div id="uploader-demo">
    <!--用来存放item-->
    <div id="sg_file_list" class="uploader-list"></div>
    <div id="$uploader">选择图片</div>
    <input type="hidden" name="poster" id="sg_uploader_input" value="">
</div>
<link rel="stylesheet" type="text/css" href="/base/plugins/webuploader/webuploader.css">
<script type="text/javascript" src="/base/plugins/webuploader/webuploader.js"></script>
<style type="text/css">
...
<script type="text/javascript">
    ...
    $uploader.on( 'fileQueued', function( file ) {
        var sg_li = $('<div id="' + file.id + '" class="file-item thumbnail">' + '<img>' + '<div class="info">' + file.name + '</div>' + '</div>'),
            sg_img = sg_li.find('img');
        $('#sg_file_list').append( sg_li );
        $uploader.makeThumb( file, function( error, src ) {
            if ( error ) {
                sg_img.replaceWith('<span>不能预览</span>');
                return;
            }
            sg_img.attr( 'src', src );
    ...
    $uploader.on( 'uploadSuccess', function( file, response ) {
        $('#sg_uploader_input').val(response.fileurl);
        
        $( '#'+file.id ).addClass('upload-state-done');
    });
    $uploader.on( 'uploadError', function( file ) {
        var sg_li = $( '#'+file.id ),
            sg_error = sg_li.find('div.error');
        if ( !sg_error.length ) {
            sg_error = $('<div class="error"></div>').appendTo( sg_li );
        }
        sg_error.text('上传失败');
    });
...
<script type="text/javascript">
    ...
    $uploader.on('fileQueued', function(file) {
        ...
        var url = "$url";
        console.log($uploader.getStats(), url);
        var status = $uploader.getStats();
        if(url != 'empty' && status.uploadFailNum == 0) {
            window.location = url;
        }
