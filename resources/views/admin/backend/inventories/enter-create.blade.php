    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.status == 1) {
                ...
                $this.parent().next().next().next().next().find('input').val(data.price);
            }
        },
        error: function (data) {
...
function updateName(val) {
    // names = "{{ $names }}";
    // names = names.split(',');
    $this = $(event.target).parent().next().next().find('input');
    // $this.autocomplete({
    //     minLength: 0,
    //     source: names
    // });
...
function computeMoneyWhenNum(val) {
    price = parseFloat($(event.target).parent().next().find('input').val());
    num = parseInt(val);
...
function computeMoneyWhenPrice(val) {
    ...
    num = parseInt($(event.target).parent().prev().find('input').val());
    total_money = price * num;
    if(isNaN(total_money)) {
        total_money = 0;
    }
    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="ibox float-e-margins">
                ...
                <form id="inventory_form" method="POST" action="{{ U('Backend/Inventory/enterSaveAll') }}" class="form-horizontal">
                	{{ csrf_field() }}
	                <div class="ibox-content">
	                    <div class="row">
                            <div class="form-group">
                                <div class="control-label col-sm-1">采购时间</div>
                                <div class="col-sm-2">
                                    <input type="text" class="form-control input-sm" name="time" id="enter_time">
                                </div>
                                <div class="control-label col-sm-1">采购人</div>
                                <div class="col-sm-2">
...
<table>
    <tr>
        ...
        <td class="sg-td" data-name="is_in">
...
$(function() {
    ...
    $('#cancel_table').on('click', function() {
    	window.location = "{{ U('Backend/Inventory/enterIndex') }}";
    });
