			...
			<div class="ibox float-e-margins">
				<div class="ibox-title">
					<div class="btn-group">
						...
						<div class="btn btn-primary disabled btn-lg">库存——出库</div>
					</div>
						...
						<form class="form-horizontal" role="form">
							<div class="form-group">
								...
								<div class="col-sm-2 control-label pull-left">
									<a href="{{ U('Backend/Inventory/outIndex', ['quick_time' => 'one_week']) }}" class="pull-left">一周</a>
									<a href="{{ U('Backend/Inventory/outIndex', ['quick_time' => 'one_month']) }}" class="pull-left" style="margin-left: 10px; margin-right: 10px;">一月</a>
									<a href="{{ U('Backend/Inventory/outIndex', ['quick_time' => 'one_year']) }}" class="pull-left">一年</a>
								</div>
							</div>
						</form>
					</div>
...
<!-- 						<div class="col-sm-2">
							<div class="btn btn-sm btn-success pull-right" id="create_inventory">新增出库</div>
						</div> -->
					</div>
					<div id="sg-table-wrapper">
						<div class="row">
							<div class="col-sm-12">
								<label class="control-label col-sm-6">费用总计：{{ $total_money }}</label>
							</div>
						</div>
						@if($buy_classes->count() != 0)
							<table class="table table-striped table-bordered table-hover dataTable">
								<thead>
									<tr>
										<th>成本项</th>
										@foreach($buy_classes as $buy_class)
											<th>{{ $buy_class->name }}</th>
										@endforeach
									</tr>
								</thead>
								<tbody>
									<tr>
										<td>一周成本</td>
										@foreach($buy_classes as $buy_class)
											<td>{{ $buy_class->total_money }}</td>
										@endforeach
									</tr>
									<tr>
										<td>规定区间</td>
										@foreach($buy_classes as $buy_class)
											@if($buy_class->cost_range == null)
												<td>无</td>
											@else
												<td>{{ $buy_class->cost_range->range_min . '%到' . $buy_class->cost_range->range_max . '%' }}</td>
											@endif
										@endforeach
									</tr>
									<tr>
										...
										@foreach($buy_classes as $buy_class)
											@if($total_money != 0)
												<td>{{ (double)$buy_class->total_money / $total_money * 100 . '%' }}</td>
											@else
												<td>无</td>
											@endif
										@endforeach
									...
									<tr>
										...
										@foreach($buy_classes as $buy_class)
											@if($buy_class->cost_range != null)
												<td class="danger-color">{{ $buy_class->cost_range->info }}</td>
											@else
												<td class="success-color">正常</td>
											@endif
										@endforeach
									</tr>
								</tbody>
	                    ...
	                    <table class="table table-striped table-bordered table-hover dataTable" id="inventory-table">
	                        ...
	                        <tbody>
	                            @if(count($inventories) == 0)
	                                <tr>
	                                    <td colspan="15" class="sg-centered">暂无出库信息！</td>
	                                </tr>
	                            @else
...
$(function() {
	...
	$('#create_inventory').on('click', function() {
		url = "{{ U('Backend/Inventory/createOut') }}";
		window.location = url;
	});
	...
	$('#filter-select').on('click', function() {
		url = "{{ U('Backend/Inventory/outIndex') }}" + '?scene=' + $('#select-scene').val() + '&buy_class=' + $('#select-buy-class').val() + '&begin_time=' + $('#sort_begin_time').val() + '&end_time=' + $('#sort_end_time').val();
		window.location = url;
	});

	$('#search-btn').on('click', function() {
		url = "{{ U('Backend/Inventory/outIndex') }}" + '?name=' + $('#search-name').val();
		window.location = url;
	});

	$('#inventory-table').on('click', '.btn-edit', function() {
		window.location = "{{ U('Backend/Inventory/editOut') }}" + '?id=' + $(this).attr('data-id');
	...
	$('#delete-confirm-btn').on('click', function() {
		url = "{{ U('Backend/Inventory/deleteOut') }}" + '?id=' + $('#delete-modal').attr('data-id');
